...
impl TryFrom<ProtobufPipeMessage> for PipeMessage {
    ...
    fn try_from(protobuf_pipe_message: ProtobufPipeMessage) -> Result<Self, &'static str> {
        let source = match (
            ...
            protobuf_pipe_message.web_source_id,
        ) {
            ...
            (Some(ProtobufPipeSource::Keybind), _, _, _) => PipeSource::Keybind,
            (Some(ProtobufPipeSource::Web), _, _, Some(web_source_id)) => {
                PipeSource::Web(web_source_id)
            },
...
impl TryFrom<PipeMessage> for ProtobufPipeMessage {
    ...
    fn try_from(pipe_message: PipeMessage) -> Result<Self, &'static str> {
        let (source, cli_source_id, plugin_source_id, web_source_id) = match pipe_message.source {
            ...
            PipeSource::Keybind => (ProtobufPipeSource::Keybind as i32, None, None, None),
            PipeSource::Web(web_client_id) => (
                ProtobufPipeSource::Web as i32,
                None,
                None,
                Some(web_client_id),
            ),
